!   Simple demo of a 2D line plot - parameterised line, continuous display
!
...
!!  integer, parameter :: pl_test_flt = kind(1.0d0)
    ...
    do i = 1,50
        param = 2.0_pl_test_flt * (i-1)

        ! Plot the data that was prepared above.
        y = 50.0 - ymax * x**2 + param * x
        call plline( x, y )

        if ( i < 50 ) then
            call sleepqq( 250 )
            call plcol0( 0 )
            call plfill( [xmin, xmax, xmax, xmin], [ymin, ymin, ymax, ymax] )
            call plcol0( 1 )
            !call plline( [xmin, xmax, xmax, xmin, xmin], &
            !             [ymin, ymin, ymax, ymax, ymin] )
            call plaxes( xmin, ymin, "acst", 0.0_pl_test_flt, 0, "acst", 0.0_pl_test_flt, 0 )
        endif
    enddo
